...
func (jss *JetstreamStore) Connect() error {
	...
	startSeq, _ := jss.wrappedStore.Get(sequenceKey)
...
loop:
	for {
		...
		case jss.putEvent:
			...
			// Persist locally
			if err := jss.wrappedStore.Put(putMessage.Key, putMessage.Value); err != nil {
				log.Errorf("Could not persist put: %s", err)
				continue loop
			}

			// Persist sequence number to local store
			if err := jss.wrappedStore.Put(sequenceKey, meta.Sequence.Stream); err != nil {
		...
		case jss.deleteEvent:
			...
			// Persist locally
			if err := jss.wrappedStore.Delete(deleteMessage.Key); err != nil {
