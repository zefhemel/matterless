...
func TestCheck(t *testing.T) {
	...
	assert.Equal(t, "no 'path' defined for endpoint", results.APIs[0].Error())
	assert.Equal(t, "invalid 'schedule' format: Expected 5 to 6 fields, found 1: bla", results.Crons[0].Error())
}
