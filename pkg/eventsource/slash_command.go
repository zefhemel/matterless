...
func (s *SlashCommandSource) Start() error {
	...
	for _, cmd := range customCommands {
		if cmd.Trigger == s.def.Trigger {
			...
			cmd.AutoComplete = s.def.AutoComplete
			cmd.AutoCompleteDesc = s.def.AutoCompleteDesc
			cmd.AutoCompleteHint = s.def.AutoCompleteHint
	...
	if !foundExisting {
		...
		cmd, resp := s.adminClient.CreateCommand(&model.Command{
			...
			Method:           "P",
			AutoComplete:     s.def.AutoComplete,
			AutoCompleteHint: s.def.AutoCompleteHint,
			AutoCompleteDesc: s.def.AutoCompleteDesc,
...
func (s *SlashCommandSource) ExtendDefinitions(defs *definition.Definitions) {
	...
	defs.APIs = append(defs.APIs, &definition.EndpointDef{
		Path:     fmt.Sprintf("/%s/callback", s.sourceName),
		Methods:  []string{"POST"},
		Function: "",
